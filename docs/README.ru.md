...
- **Резервный Groq Whisper** — Автоматический переход на [Groq](https://groq.com/) Whisper при исчерпании месячного
  лимита Wit.ai
- **Система кредитов** — Монетизация через Telegram Stars с балансом кредитов для каждого пользователя
...
- **Автокатегоризация** — Автоматическая классификация заметок по категориям с помощью Claude Haiku
...
│   └── groq_client.py   # Клиент Groq Whisper
...
├── categorization.py    # Автокатегоризация заметок с Claude
└── main.py              # Точка входа
```
