...
- **AI-очистка текста** — Опциональная очистка транскрипций через LLM (удаление слов-паразитов, расстановка пунктуации). При выключенной очистке в `/settings` ответ в Telegram остаётся сырым, но в Obsidian текст всё равно очищается.
...
- **Выбор репозитория** — При подключении можно выбрать существующий репозиторий GitHub или создать новый (с любым именем)
